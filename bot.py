...
def check_and_configure_config():
    ...
    while not config["playerok"]["api"]["token"]:
        print(f"\n{Fore.WHITE}Введите {Fore.LIGHTRED_EX}токен {Fore.WHITE}вашего Playerok аккаунта. Его можно узнать из Cookie-данных, воспользуйтесь расширением Cookie-Editor."
              f"\n  {Fore.WHITE}· Пример: eyJhbGciOiJIUzI1NiIsIDR5cCI1dDSVpXCCJ9.eyJzdWIiOiIxZWUxMzg0Ni...")
        token = input(f"  {Fore.WHITE}↳ {Fore.LIGHTWHITE_EX}").strip()
        if is_token_valid(token):
            config["playerok"]["api"]["token"] = token
            sett.set("config", config)
            print(f"\n{Fore.LIGHTMAGENTA_EX}Токен успешно сохранён в конфиг.")
        else:
        ...
        if is_user_agent_valid(user_agent):
            ...
            sett.set("config", config)
            print(f"\n{Fore.LIGHTMAGENTA_EX}User Agent успешно сохранён в конфиг.")
        else:
        ...
        print(f"\n{Fore.WHITE}Введите {Fore.LIGHTRED_EX}IPv4 Прокси {Fore.WHITE}в формате user:password@ip:port или ip:port, если он без авторизации. Если вы не знаете что это, или не хотите устанавливать прокси - пропустите этот параметр, нажав Enter."
        ...
        if is_proxy_valid(proxy):
            ...
            sett.set("config", config)
            print(f"\n{Fore.LIGHTMAGENTA_EX}Прокси успешно сохранён в конфиг.")
        else:
    ...
    while not config["telegram"]["api"]["token"]:
        ...
        if is_tg_token_valid(token):
            ...
            sett.set("config", config)
            print(f"\n{Fore.LIGHTMAGENTA_EX}Токен Telegram бота успешно сохранён в конфиг.")
        else:
            print(f"\n{Fore.LIGHTRED_EX}Похоже, что вы ввели некорректный токен. Убедитесь, что он соответствует формату и попробуйте ещё раз.")
    ...
    while not config["telegram"]["bot"]["password"]:
        ...
        if is_password_valid(password):
            ...
            sett.set("config", config)
            print(f"\n{Fore.LIGHTMAGENTA_EX}Пароль успешно сохранён в конфиг.")
        else:
...
if __name__ == "__main__":
    ...
    print(f"\n   {Fore.LIGHTRED_EX}Ваш бот словил непредвиденную ошибку и был выключен."
          f"\n   {Fore.WHITE}Пожалуйста, напишите в Telegram разработчика {Fore.LIGHTWHITE_EX}@supervaizir{Fore.WHITE}, для уточнения причин")
    raise SystemExit(1)