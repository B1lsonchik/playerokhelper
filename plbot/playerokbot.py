...
class PlayerokBot:
    ...
    def __init__(self):
        ...
        self.logger = getLogger(f"playerok.playerok")
    ...
    def log_new_message(self, message: types.ChatMessage, chat: types.Chat):
        ...
        self.logger.info(f"{Fore.LIGHTRED_EX}{ch_header.replace(chat_user, f'{Fore.LIGHTCYAN_EX}{chat_user}')}")
        self.logger.info(f"{Fore.LIGHTRED_EX}│ {Fore.LIGHTWHITE_EX}{message.user.username}:")
        ...
        for raw_line in text.split("\n"):
            if not raw_line.strip():
                self.logger.info(f"{Fore.LIGHTRED_EX}│")
                continue
            wrapped_lines = textwrap.wrap(raw_line, width=max_width)
            for wrapped in wrapped_lines:
                self.logger.info(f"{Fore.LIGHTRED_EX}│ {Fore.WHITE}{wrapped}")
    ...
    async def run_bot(self):
        self.logger.info(f"{Fore.MAGENTA}Playerok Helper запущен")
        self.logger.info("")
        self.logger.info(f"{Fore.LIGHTRED_EX}───────────────────────────────────────")
        self.logger.info(f"{Fore.LIGHTRED_EX}Информация об аккаунте:")
        ...
        self.logger.info(f" * Активных продаж: {Fore.LIGHTWHITE_EX}{self.playerok_account.profile.stats.deals.outgoing.total - self.playerok_account.profile.stats.deals.outgoing.finished}")
        self.logger.info(f"{Fore.LIGHTRED_EX}───────────────────────────────────────")
        self.logger.info("")
        if self.config["playerok"]["api"]["proxy"]:
            ...
            self.logger.info(f"{Fore.LIGHTRED_EX}───────────────────────────────────────")
            self.logger.info(f"{Fore.LIGHTRED_EX}Информация о прокси:")
            ...
            def endless_loop():
                while True:
                    try:
                        ...
                        set_title(f"Playerok v{VERSION} | {self.playerok_account.username}: {balance}₽")
        ...
        self.logger.info(f"Логгер событий запущен")
