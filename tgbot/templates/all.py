...
def log_text(title: str, text: str, by: str = "playerok"):
    txt = textwrap.dedent(f"""
        <b>{title}</b>
        \n{text}
        \n<i>{by}</i>
...
def menu_text():
    txt = textwrap.dedent(f"""
        ...
        <b>Playerok Helper</b> v{VERSION}
        Бот-помощник для Playerok

        <b>Ссылки:</b>
        ┣ <b>@supervaizir</b> — главный и единственный разработчик
        ┗ <b>@PlayerokHelper_news</b> — канал, где публикуются новости
...
def menu_kb():
    rows = [
        ...
        [
        InlineKeyboardButton(text="👨‍💻 Разработчик", url="https://t.me/supervaizir"),
        InlineKeyboardButton(text="📢 Наш канал", url="https://t.me/PlayerokHelper_news"),
        ]
    ]
    kb = InlineKeyboardMarkup(inline_keyboard=rows)
    return kb
...
def settings_kb():
    rows = [
        ...
        InlineKeyboardButton(text="📶 Прокси", callback_data=calls.SettingsNavigation(to="conn").pack()),
