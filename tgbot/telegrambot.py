...
logger = logging.getLogger(f"playerok.telegram")
...
class TelegramBot:
    ...
    async def set_short_description(self):
        try:
            short_description = textwrap.dedent(f"""
                Playerok Helper — Современный бот-помощник для Playerok 🟦
                ┕ Канал — @PlayerokHelper_news
                ┕ Разработчик — @supervaizir
            """)
    ...
    async def set_description(self):
        try:
            description = textwrap.dedent(f"""
                Playerok — Бесплатный современный бот-помощник для Playerok 🟦
    ...
    async def run_bot(self):
        ...
        logger.info(f"{Fore.LIGHTRED_EX}Telegram бот {Fore.CYAN}@{me.username} {Fore.LIGHTRED_EX}запущен")
